...
The simulated camera source is an existing **DeepStream sample video**, read
directly from its install path at run time:
...
1. **Size.** The file is ~33 MiB (34,952,625 bytes). Git stores binaries badly
   and every future revision would be stored in full.
2. **It is already on every target machine.** Any Jetson with DeepStream
   installed already has this exact file, so committing it adds nothing.
